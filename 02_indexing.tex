...
% Title information
\title[IR Indexing]{Information Retrieval Indexing}
\subtitle{Lecture 2: Posting Lists, Compression \& ANN Indexing}
...
\begin{enumerate}
    \item Introduction to IR: VSM \& Boolean Retrieval \checkmark
...
\begin{itemize}
    \item Document ID \cpause
    \item Term frequency (for ranking) \cpause
    \item Positions (for phrase queries, proximity) \cpause
...
\begin{itemize}
    \item Can't score all documents (millions/billions) \cpause
    \item Must process posting lists cleverly \cpause
...
\textbf{Benefits:}
\begin{itemize}
    \item \textbf{Early termination:} Stop after processing top-$k$ high-impact postings \cpause
    \item \textbf{Safe to skip:} Low-impact docs unlikely to reach top-$k$ \cpause
...
\textbf{Drawback:}
\begin{itemize}
    \item Can't use skip pointers (not sorted by docID) \cpause
