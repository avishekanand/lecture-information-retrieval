...
% ====================================================================
\section{Part 0 --- From Rankings to Direct Answer}
% ====================================================================
...
\begin{frame}[shrink=10]{Evaluation: BERTScore}
% \irStage{EVALUATION}
\textbf{Using contextual embeddings to evaluate semantic similarity.}
...
\begin{frame}[shrink=10]{Evaluation: BERTScore}
\irStage{EVALUATION}
\textbf{Semantic matching using contextual embeddings.}
...
\begin{block}{BERTScore (Zhang et al., 2020)}
\begin{itemize}
  \item Represent tokens with contextual embeddings
  \item Compute pairwise cosine similarity
  \item Greedily match each token to its most similar counterpart
  \item Derive precision, recall, and an embedding-based F-score
\end{itemize}

\[
F_{\mathrm{BERT}}=
\frac{2 P_{\mathrm{BERT}} R_{\mathrm{BERT}}}
{P_{\mathrm{BERT}} + R_{\mathrm{BERT}}}
\]

\footnotesize Optional IDF weighting can emphasize informative tokens.
\end{block}
\end{column}

\begin{column}{0.38\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/bertscore.pdf}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{DeRIVeR: Why Decouple?}
% \irStage{PARADIGM}
\textbf{Independent modules offer flexibility and efficiency.}

\vspace{0.7em}
\begin{itemize}
  \item \textbf{Independent Optimization}: Each role can be independently optimised, swapped, or scaled.
  \item \textbf{Model Variety}: The planner can be a small LLM; the retriever can be BM25; the synthesiser can be GPT-4.
  \item \textbf{Task Adaptation}: Easily adjust individual modules based on varying task requirements.
\end{itemize}
\end{frame}
...
\begin{frame}{Query Planning: Cost Trade-off}
% \irStage{QUERY PLANNING}
\textbf{Added complexity brings added latency and cost.}

\vspace{0.7em}
\begin{block}{Considerations}
\begin{itemize}
  \item Each planning step adds latency and token cost.
  \item Use the simplest approach that achieves sufficient retrieval recall.
  \item Measure baseline recall with single-shot before adding planning complexity.
\end{itemize}
\end{block}
\end{frame}
...
\begin{frame}{Relationship to DeRIVeR}
% \irStage{ADVANCED}
\textbf{IR-CoT is a concrete instantiation of the DeRIVeR paradigm.}

\vspace{0.7em}
\begin{itemize}
  \item \textbf{Dynamic Planning:} Chain-of-thought generates the query plan on-the-fly rather than as a separate planning step.
  \item \textbf{Integrated Execution:} The LLM serves both as the reasoning module and implicitly as the planner.
\end{itemize}
\end{frame}

\begin{frame}[shrink=10]{Self-RAG: Selective Retrieval with Critique Tokens}
% \irStage{ADVANCED}
