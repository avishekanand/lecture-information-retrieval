...
\begin{frame}[shrink=10]{BM25 Component 3: Length Normalization}
...
IDF difference drives discrimination: Rare terms dominate scoring (Matches on ``quantum'' worth $\sim$15$\times$ matches on ``system''). Common terms contribute little or negatively
...
\begin{frame}[shrink=5]{The Vocabulary Mismatch Problem}
...
\vspace{0.5em}
\cpause
\begin{exampleblock}{Core Idea (Gao et al., 2022)}
...
\begin{frame}[shrink=20]{Strategy 4: GenQR (Generative Query Reformulation) --- Intuition}
...
\begin{block}{Core Idea (Wang et al., 2022)}
...
% \begin{frame}[shrink=22]{Strategy 4: GenQR --- Method (Prompting \& Fine-tuning)}
...
% \begin{frame}[shrink=22]{Strategy 4: GenQR --- Pipeline Details, When It Works, Risks}
